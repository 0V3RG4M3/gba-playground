...
def generate_mmio_patch(reg: gba_mmio.Register, subpatch: SubPatchBox) -> Patch:
    gridspace = GridSpace(x0=10.0, y0=10.0, width=32.0, height=32.0)
    ...
    # Add title box
    patch.XY = 0, 1
    patch.add_box(MessageBox(message=f"{reg.NAME}"))
    ...
    for k, field in enumerate(fields):
        ...
        patch.add_box(label_box)

        patch.XY = X0 + N - 1 - k, Y0 + k
...
def generate_all_sound_maxpat() -> None:
    for register in gba_mmio.registers:
        ...
        with open(f"..\\maxGBA\\mg0.gba_{register_name}-{type_name}.maxpat", "w", encoding="utf-8") as f:
