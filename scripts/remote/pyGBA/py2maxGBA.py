...
class MaxBox:
    ...
    def update_presentation_rect(self) -> None:
        self.box_dict["box"]["presentation_rect"] = self.box_dict["box"]["patching_rect"][:]
    ...
    def patching_rect_xy(self, value: list[float]):
        self.box_dict["box"]["patching_rect"][:2] = value
        self.update_presentation_rect()
    ...
    def patching_rect_wh(self, value: list[float]):
        self.box_dict["box"]["patching_rect"][2:4] = value
        self.update_presentation_rect()

    @property
    def patching_rect_w(self) -> float:
        return self.box_dict["box"]["patching_rect"][2]

    @patching_rect_w.setter
    def patching_rect_w(self, value: float):
        self.box_dict["box"]["patching_rect"][2] = value
        self.update_presentation_rect()

    @property
    def patching_rect_h(self) -> float:
        return self.box_dict["box"]["patching_rect"][3]

    @patching_rect_h.setter
    def patching_rect_h(self, value: float):
        self.box_dict["box"]["patching_rect"][3] = value
        self.update_presentation_rect()
    ...
    def presentation(self) -> int:
        if "presentation" not in self.box_dict["box"]:
            return int(False)
        return self.box_dict["box"].get("presentation", int(False))

    @presentation.setter
    def presentation(self, value: bool):
        self.box_dict["box"]["presentation"] = int(value)
...
class InletBox(MaxBox):
    ...
    def __init__(self, hint: str, comment: str, index: int) -> None:
        ...
        self.id_ = f"AUTOGENERATED-{self.__class__.__name__}-{self.__class__.Counter}"
        print(index, self.id_)
        self.box_dict = {
            "box": {
                "annotation": hint,
                "comment": comment,
...
class IGridSpace:
    ...
    def compute_wh(self, W, H) -> tuple[float, float]:
        """
        Same as compute_xy, but for without applying the offset. (vector) Usefull to compute width and height
        :param W:
        :param H:
        :return:
        """
...
class Patch:
    ...
    def add_box(self, box: MaxBox, W=None, H=None) -> None:
        box.patching_rect_xy = self.gridspace.compute_xy(self.X, self.Y)
        if W is not None:
            box.patching_rect_w = self.gridspace.compute_wh(W, 0)[0]

        if H is not None:
            box.patching_rect_h = self.gridspace.compute_wh(0, H)[1]
...
class SubPatchBox(MaxBox):
    ...
    def __init__(self, name: str, maxpatch: Patch) -> None:
        ...
        color = [0.611764705882353, 1.0, 0.43529411764705883, 1.0]
        self.box_dict = {
            "box": {
                "maxclass": "newobj",
                "color": color,
                ...
                "patcher": self.maxpatch.dict["patcher"],
                "saved_object_attributes": {
                    "description": "",
                    "digest": "",
                    "globalpatchername": "",
